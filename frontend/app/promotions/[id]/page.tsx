...
export default async function PromotionPage({ params }: Readonly<PromotionPageProps>) {
  const { id } = await params
  const promotion = promotionalOffers.find((p) => p.id === id)

  if (!promotion) {
    notFound()
  }

  const timeLeft = new Date(promotion.expiresAt).getTime() - Date.now()
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {promotion.includedItems.map((item) => (
                <div key={item.name} className="text-center p-6 bg-gray-50 rounded-2xl hover:bg-gray-100 transition-colors">
