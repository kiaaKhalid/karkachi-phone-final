...
export default function LoginPage() {
  ...
  const { login: userLogin, googleLogin, isAuthenticated: isUserAuthenticated, user: regularUser, recheckSession } = useAuth();
  ...
  const syncLocalCartToServer = async () => {
    try {
      ...
      const response = await fetch(`${apiUrl}/carts/items/all`, {
        method: "POST",
        headers: {
    ...
    try {
      ...
      if (userResult.success && userResult.user) {
        ...
        try {
          ...
          console.warn("Ignored cart sync error during mock login");
        }
  ...
  const handleGoogleSuccess = async (credentialResponse: CredentialResponse) => {
    if (!credentialResponse.credential) {
      toast({ title: "Erreur Google", description: "Jeton manquant", variant: "destructive" });
      return;
    }
    ...
    try {
      const userResult = await googleLogin(credentialResponse.credential);

      if (userResult.success && userResult.user) {
        try {
          await syncLocalCartToServer();
        } catch (e) {
          console.warn("Ignored cart sync error during google login");
        }
      ...
      setError(userResult.error || "Échec de l'authentification Google");
    } catch (err) {
