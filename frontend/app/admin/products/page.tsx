...
export default function AdminProducts() {
  ...
  const fetchProducts = async (page = 0, size = 25) => {
    try {
      ...
      // Use brand.name from API response
      const productsWithBrand = data.items.map((product: any) => ({
        ...product,
  ...
  const sortedProducts = [...filteredProducts].sort((a, b) => {
    switch (sortBy) {
      ...
      case "rating-low":
        return Number.parseFloat(a.rating) - Number.parseFloat(b.rating)
      default:
  ...
  const renderTableContent = () => {
    ...
    return sortedProducts.map((product) => (
      <TableRow key={product.id}>
        ...
        <TableCell>
          <span
            className={`font-medium ${product.stock <= 10 ? "text-red-500" : ""} ${product.stock > 10 && product.stock <= 50 ? "text-yellow-500" : ""} ${product.stock > 50 ? "text-green-500" : ""}`.trim()}
          >
            {product.stock}
          </span>
        </TableCell>
        ...
        <TableCell>
          <div className="flex items-center gap-1">
            <span>{Number.parseFloat(product.rating).toFixed(1)}</span>
        ...
        <TableCell className="text-right">
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              ...
              <DropdownMenuSeparator />
              <DropdownMenuItem onClick={() => handleViewProduct(product.id)}>View Details</DropdownMenuItem>
