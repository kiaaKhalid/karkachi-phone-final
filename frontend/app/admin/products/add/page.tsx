...
export default function AddProductPage() {
  ...
  useEffect(() => {
    ...
    const originalPriceNum = Number.parseFloat(formData.originalPrice) || 0
  ...
  const fetchBrands = async () => {
    try {
      ...
      const response = await fetch(`${apiUrl}/admin/brands?limit=1000`, {
        method: "GET",
        headers: {
  ...
  const fetchCategories = async () => {
    try {
      ...
      const response = await fetch(`${apiUrl}/admin/categories?limit=1000`, {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${localStorage.getItem("auth_token")}`,
      ...
      const fullData = await response.json()
      const data: CategoryChoix[] = fullData.items || []
      setCategories(data)
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const requestData = {
        ...
        isProductPhare: formData.isProductphares,
        ...
        colors: formData.hasColors ? formData.colors.map((color) => ({ name: color.name, hex: color.hex, imageUrl: color.imageUrl })) : [],
      }
      ...
      // result might not have .success if it's returning the object directly (NestJS default)
              ...
              <CardHeader>
                <div className="flex items-center justify-between">
                  ...
                  <Switch
                    ...
                    onCheckedChange={(checked) => handleInputChange("hasColors", checked)}
                  />
                </div>
              </CardHeader>
              {formData.hasColors && (
                      ...
                      >
                        ...
                        <div className="flex-1 flex items-center gap-2">
                          {color.imageUrl ? (
                            ...
                            <div className="flex items-center gap-2">
                              <Label htmlFor={`color-img-${index}`} className="cursor-pointer bg-white dark:bg-gray-700 px-3 py-1.5 border rounded text-sm hover:bg-gray-50">
              ...
              <CardContent className="space-y-4">
                <div className="space-y-2">
                  ...
                  <DragDropImageUpload
                    ...
                    onImageRemove={() => handleInputChange("mainImage", "")}
