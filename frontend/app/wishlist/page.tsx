...
import { HeartCrack, Filter, ShoppingBag, Trash2, ArrowLeft } from "lucide-react"
...
export default function WishlistPage() {
  ...
  const fetchWishlist = async () => {
    try {
      ...
      const response = await fetch(`${url}/wishlists?${params}`, {
        method: "GET",
        headers: {
  ...
  const handleClearWishlist = async () => {
    try {
      ...
      const response = await fetch(`${url}/wishlists/clear`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`,
        },
      ...
      toast({
        title: "Error",
        description: err instanceof Error ? err.message : "Failed to clear wishlist. Please try again.",
        variant: "destructive",
      })
    } finally {
  ...
  const handleRemoveFromWishlist = async (productId: string) => {
    try {
      ...
      const response = await fetch(`${url}/wishlists/${productId}`, {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
  ...
  // Calcul des statistiques (Currently Unused)
  // const stats = { ... }
            ...
            <div className="text-center space-y-4">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-8 max-w-2xl mx-auto">
                {Array.from({ length: 4 }).map((_, i) => (
                  <div key={crypto.randomUUID()} className="bg-white/10 backdrop-blur-sm rounded-xl p-4 text-center">
        ...
        {wishlistItems.length > 0 && (
          <div className="fixed bottom-16 left-0 right-0 z-40 bg-background/95 backdrop-blur-md p-4 border-t border-border/10 shadow-lg md:hidden animate-slide-up">
            <SlideToAction
              ...
              actionText="Glisser pour Ajouter au Panier"
