...
export function generateTrackingQRCode(trackingNumber: string, orderId: string): string {
  ...
  const svgQR = `data:image/svg+xml;base64,${btoa(svgContent)}`
  return svgQR
}
...
export function parseQRCodeData(qrData: string): { trackingCode: string; orderId: string; timestamp: number } | null {
  try {
    ...
    // If JSON parsing fails, log it and try the old format
    console.debug("JSON parse failed, trying legacy format", error_)
    try {
      ...
      console.debug("Legacy parse failed", fallbackError)
