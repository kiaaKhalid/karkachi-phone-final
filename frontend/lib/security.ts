...
function getSubtleCrypto(): SubtleCrypto | null {
  return globalThis?.window?.crypto?.subtle ?? null;
}
...
async function getCryptoKey(): Promise<CryptoKey> {
  ...
  const keyBuffer = Uint8Array.from(
    ENCRYPTION_KEY_HEX.match(/.{2}/g)!.map(byte => Number.parseInt(byte, 16))
  );
...
export async function encryptData(text: string): Promise<string> {
  const subtle = getSubtleCrypto();
  if (!subtle) {
    // Dev fallback for non-secure contexts (IP origins) where crypto.subtle is unavailable.
    return `plain:${encodeUtf8ToBase64(text)}`;
  }

  const encoder = new TextEncoder();
  const data = encoder.encode(text);
  const iv = globalThis.window.crypto.getRandomValues(new Uint8Array(12)); // 12 bytes pour GCM
  ...
  const ivB64 = btoa(Array.from(iv).map(b => String.fromCodePoint(b)).join(""));
  const encryptedB64 = btoa(Array.from(new Uint8Array(encryptedBuffer)).map(b => String.fromCodePoint(b)).join(""));
...
export async function decryptData(encryptedText: string): Promise<string> {
  if (encryptedText.startsWith("plain:")) {
    return decodeBase64ToUtf8(encryptedText.slice("plain:".length));
  }
  ...
  const subtle = getSubtleCrypto();
  if (!subtle) {
    throw new Error("Web Crypto API non disponible pour dechiffrement");
  }
  
  const iv = Uint8Array.from(atob(ivB64), c => c.codePointAt(0)!);
  const encrypted = Uint8Array.from(atob(encryptedB64), c => c.codePointAt(0)!);
  const key = await getCryptoKey();
...
export async function generateHash(data: string): Promise<string> {
  const subtle = getSubtleCrypto();
  if (!subtle) {
    throw new Error("Web Crypto API non disponible pour hash");
  }
  const encoder = new TextEncoder();
  const dataBuffer = encoder.encode(data);
  const hashBuffer = await subtle.digest(HASH_ALGORITHM, dataBuffer);
...
export class RateLimiter {
  private readonly attempts: Map<string, { count: number; resetTime: number }> = new Map();
