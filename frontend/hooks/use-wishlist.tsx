...
export function WishlistProvider({ children }: Readonly<{ children: ReactNode }>) {
  ...
  const fetchWishlist = async () => {
    ...
    try {
      ...
      const response = await fetch(`${apiUrl}/wishlists?limit=100`, {
  ...
  const removeFromWishlist = async (productId: string) => {
    ...
    try {
      ...
      const response = await fetch(`${apiUrl}/wishlists/${productId}`, {
        method: "DELETE",
        headers: { 
          Authorization: `Bearer ${token}`,
        },
  ...
  const clearWishlist = async () => {
    ...
    try {
      ...
      const response = await fetch(`${apiUrl}/wishlists/clear`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const value = useMemo<WishlistContextType>(() => ({
    ...
    refreshWishlist,
    isLoading,
  }), [wishlist, addToWishlist, removeFromWishlist, isInWishlist, clearWishlist, refreshWishlist, isLoading]);
