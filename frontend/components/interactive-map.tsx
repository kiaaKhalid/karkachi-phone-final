...
function MapMarker({
  ...
  onClick,
  isActive,
}: Readonly<{ position: [number, number, number]; onClick: () => void; isActive: boolean }>) {
...
function ChangeView({ center, zoom }: Readonly<{ center: [number, number]; zoom: number }>) {
  const map = useMap()
  useEffect(() => {
...
function GoogleMap({ latitude, longitude, zoom = 13 }: Readonly<InteractiveMapProps>) {
...
function InteractiveMapLeaflet({ latitude, longitude, zoom = 13 }: Readonly<InteractiveMapProps>) {
...
export default function InteractiveMap({ latitude, longitude, zoom = 13 }: Readonly<InteractiveMapProps>) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-center gap-4 mb-6">
        <Button
          ...
          variant={showGoogleMap ? "outline" : "default"}
