...
export default function EditProductPopup({ isOpen, onClose, productId, initialMode = "view" }: EditProductPopupProps) {
  ...
  const fetchProductDetails = async (id: string) => {
    try {
      ...
      if (fullResponse.error || fullResponse.statusCode >= 400) {
        throw new Error(fullResponse.message || "Failed to fetch product details")
      }

      const apiData = fullResponse
      const mappedProduct: DetailedProductAdminDTO = {
        ...
        isProductphares: apiData.isProductPhare ?? false,
        ...
        imageIds: Array.isArray(apiData.images) ? apiData.images.map((img: any) => img.id) : [],
        imageUrls: Array.isArray(apiData.images) ? apiData.images.map((img: any) => img.url) : [],
        primaryImageUrl: apiData.image,
        specificationIds: Array.isArray(apiData.specs) ? apiData.specs.map((spec: any) => spec.id) : [],
        specificationNames: Array.isArray(apiData.specs) ? apiData.specs.map((spec: any) => spec.key) : [],
        specificationValues: Array.isArray(apiData.specs) ? apiData.specs.map((spec: any) => spec.value) : [],
        hasColors: apiData.hasColors ?? false,
        colors: Array.isArray(apiData.colors) ? apiData.colors : [],
      }
  ...
  const fetchBrands = async () => {
    try {
      ...
      const response = await fetch(`${baseUrl}/admin/brands?limit=1000`, {
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${localStorage.getItem("auth_token")}`,
        },
      ...
      const mappedBrands = (fullData.items || []).map((b: any) => ({
        id: b.id,
        name: b.name,
        logo: b.logo,
      }))
  ...
  const fetchCategories = async () => {
    try {
      ...
      const response = await fetch(`${baseUrl}/admin/categories?limit=1000`, {
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${localStorage.getItem("auth_token")}`,
        },
  ...
  const addColor = () => {
    if (newColor.name && newColor.hex) {
      setFormData((prev) => ({
        ...prev,
        colors: [...(prev.colors || []), { name: newColor.name, hex: newColor.hex }],
      }))
    ...
    try {
      ...
      if (response.ok) {
        ...
        setFormData((prev) => {
          ...
          newColors[index] = { ...newColors[index], imageUrl: data.url }
          return { ...prev, colors: newColors }
        })
        toast({ title: "Success", description: "Image couleur ajoutée." })
      } else {
        toast({ title: "Error", description: "Erreur lors de l'upload.", variant: "destructive" })
      }
    } catch (err) {
      toast({ title: "Error", description: "Erreur réseau.", variant: "destructive" })
    }
  }
  ...
  const initializeFormData = (productData: DetailedProductAdminDTO) => {
    setFormData({
      ...
      hasColors: productData.hasColors,
  ...
  const handleSave = async () => {
    ...
    try {
      const hasColors = formData.hasColors !== undefined ? formData.hasColors : product.hasColors;
      const colors = formData.colors !== undefined ? formData.colors : product.colors;

      if (hasColors && colors.length === 0) {
        toast({
          title: "Error",
          description: "Veuillez ajouter au moins une couleur car l'option est activée.",
          variant: "destructive",
        })
        return
      }
      ...
      const updateData: any = {
        ...
        isProductPhare: formData.isProductphares !== undefined ? formData.isProductphares : product.isProductphares,
        ...
        hasColors,
        colors: hasColors ? colors.map(c => ({ name: c.name, hex: c.hex, imageUrl: c.imageUrl })) : [],
                ...
                <Card>
                  ...
                  <CardContent className="space-y-4">
                    <div className="space-y-2">
                      ...
                      {isEditing ? (
                        <DragDropImageUpload
                          onImageUpload={(url) => updateFormField("primaryImageUrl", url)}
                          onImageRemove={() => updateFormField("primaryImageUrl", "")}
                          currentImage={formData.primaryImageUrl !== undefined ? formData.primaryImageUrl : primaryImageUrl}
                        />
                      ) : (
                        primaryImageUrl ? (
                  ...
                  <CardHeader>
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <CardDescription>Enable to specify available colors</CardDescription>
                      </div>
                      <Switch
                        disabled={!isEditing}
                        checked={isEditing ? formData.hasColors : enrichedProduct.hasColors}
                        onCheckedChange={(checked) => updateFormField("hasColors", checked)}
                      />
                    </div>
                  </CardHeader>
                  {(isEditing ? formData.hasColors : enrichedProduct.hasColors) && (
                    <CardContent className="space-y-4">
                      {isEditing && (
                        <div className="flex space-x-2">
                          <Input
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            />
                            <span className="text-sm text-gray-500 min-w-16 uppercase">{newColor.hex}</span>
                          </div>
                      ...
                      <div className="flex flex-col gap-4">
                        {(isEditing ? formData.colors : enrichedProduct.colors)?.map((color, index) => (
                          <div
                            key={`${color.name}-${index}`}
                            className="flex flex-wrap items-center gap-4 p-3 bg-gray-50 dark:bg-gray-800 border rounded-lg"
                          >
                            <div className="flex items-center gap-2">
                              <div 
                            ...
                            <div className="flex-1 flex items-center gap-2">
                              {color.imageUrl ? (
                                ...
                                <div className="flex items-center gap-2">
                                  <Label htmlFor={`edit-color-img-${index}`} className="cursor-pointer bg-white dark:bg-gray-700 px-3 py-1.5 border rounded text-sm hover:bg-gray-50">
                                    <ImageIcon className="w-4 h-4 inline mr-2" />
                                    Add Image
                                  </Label>
                                  <Input
                                    id={`edit-color-img-${index}`}
                                    type="file"
                                    accept="image/*"
                                    className="hidden"
                                    onChange={(e) => {
                                      if (e.target.files?.[0]) uploadColorImage(index, e.target.files[0])
                                    }}
                                  />
                                </div>
                              ) : null}
                      ...
                      {(isEditing ? (formData.colors || []) : (enrichedProduct.colors || [])).length === 0 && (
                        <p className="text-sm text-red-500 mt-2">
                          * You must add at least one color since the option is enabled.
                        </p>
                      )}
                    </CardContent>
                  )}
                    ...
                    {isEditing && (
                      <div className="space-y-2 pt-4 border-t">
                        ...
                        <DragDropImageUpload
                          ...
                          placeholder="Ajoutez des images supplémentaires (Glissez-déposez ou cliquez)"
                        />
                      </div>
                    )}
                  </CardContent>
