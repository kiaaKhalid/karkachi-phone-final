/* eslint-disable jsx-a11y/click-events-have-key-events, jsx-a11y/no-static-element-interactions, jsx-a11y/interactive-supports-focus, jsx-a11y/no-noninteractive-element-interactions */
"use client"
...
export default function ProductCard({
  ...
  useEffect(() => {
    if (isFlipped && globalThis.window !== undefined && globalThis.window.innerWidth < 768) {
  ...
  const handleWishlistClick = (e: React.MouseEvent) => {
    ...
    // Check for token
    const token = globalThis.window === undefined ? null : (localStorage.getItem('auth_token') || localStorage.getItem('token'))
    if (!token) {
  ...
  return (
    <div // NOSONAR - div with role="button" is required here to prevent hydration errors from nested buttons
      className="group [perspective:1000px] h-[380px] sm:h-[420px] md:h-[440px] w-full cursor-pointer text-left block"
    ...
    >
      <div className={`relative w-full h-full transition-all duration-700 [transform-style:preserve-3d] ${isHovered || isFlipped ? "[transform:rotateY(180deg)]" : ""}`}>
        ...
        <div className="absolute inset-0 w-full h-full [backface-visibility:hidden] flex flex-col bg-card rounded-2xl border border-border/50 overflow-hidden shadow-sm shadow-black/5">
          ...
          >
            {Boolean(discount && discount > 0) && <span className="badge-discount">-{discount}%</span>}
          ...
          <div className="product-card-body p-4 space-y-2 flex-none">
            ...
            <div className="flex items-center gap-2 pt-1" onMouseEnter={() => setIsHovered(false)}>
              <button
                ...
                disabled={!inStock}
                className={`flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg text-xs font-semibold ${inStock ? "bg-accent text-white hover:bg-orange-600" : "bg-muted text-muted-foreground cursor-not-allowed"
                  }`}
              >
              ...
              <button
                ...
                className={`p-2 rounded-lg border transition-colors ${liked ? "border-red-200 bg-red-50 text-red-500" : "border-border hover:border-accent/30 text-muted-foreground"
                  }`}
              >
            ...
            {product.specs && product.specs.length > 0 ? (
              <div className="space-y-2 max-h-[180px] overflow-y-auto custom-scrollbar">
                {product.specs.map((spec, index) => {
                  const key = `${product.id}-spec-${spec.key}-${index}`
                  return (
                    <div key={key} className="flex justify-between items-start gap-2 text-xs py-1 border-b border-border/30">
