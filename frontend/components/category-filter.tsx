...
export function CategoryFilter({ categories, selectedCategory, onCategoryChange }: Readonly<CategoryFilterProps>) {
  const selectCategory = (category: string) => onCategoryChange(category)
              ...
              {categories.map((category) => (
                <div key={category} className="flex items-center space-x-2">
                  <Checkbox
                    ...
                    onCheckedChange={(checked) => checked ? selectCategory(category) : unselectCategory(category)}
                  />
