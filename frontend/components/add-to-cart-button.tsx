...
export default function AddToCartButton({
  ...
  const addToCartAPI = async (productId: string, quantity = 1) => {
    ...
    const response = await fetch(`${url}/carts/items`, {
      method: "POST",
      headers: {
  ...
  const renderIconContent = () => {
    if (isAdding) return <Loader2 className="h-4 w-4 animate-spin" />
    if (justAdded) return <Check className="h-4 w-4" />
    return <ShoppingCart className="h-4 w-4" />
  }

  const renderCompactContent = () => {
    if (isAdding) return <><Loader2 className="h-3 w-3 animate-spin mr-1" />Ajout...</>
    if (justAdded) return <><Check className="h-3 w-3 mr-1" />Ajouté !</>
    return <><ShoppingCart className="h-3 w-3 mr-1" />Ajouter</>
  }

  const renderDefaultContent = () => {
    if (isAdding) return <><Loader2 className="h-4 w-4 animate-spin mr-2" />Ajout au panier...</>
    if (justAdded) return <><Check className="h-4 w-4 mr-2" />Ajouté au panier !</>
    if (product.stock === 0) return "Rupture de stock"
    return <><ShoppingCart className="h-4 w-4 mr-2" />Ajouter au panier</>
  }
